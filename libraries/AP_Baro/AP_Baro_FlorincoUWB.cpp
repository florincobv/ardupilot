...
AP_Baro_FLNCUWB::AP_Baro_FLNCUWB(AP_Baro &baro, AP_HAL::OwnPtr<AP_HAL::Device> _dev)
                                       ...
                                       AP_HAL::OwnPtr<AP_HAL::Device> _dev, bool _is_dps310)
{
    ...
    AP_Baro_FLNCUWB *sensor = new AP_Baro_FLNCUWB(baro, std::move(_dev));
    if (sensor) {
...
AP_Baro_Backend *AP_Baro_FlorincoUWB2::probe(AP_Baro &baro,
                                       AP_HAL::OwnPtr<AP_HAL::Device> _dev)
{
    // same as DPS280 but with is_dps310 set for temperature fix
    return AP_Baro_FLNCUWB::probe(baro, std::move(_dev), true);
}

/*
  handle bit width for 16 bit config registers
 */
void AP_Baro_FLNCUWB::fix_config_bits16(int16_t &v, uint8_t bits) const
{
 ...
 */
void AP_Baro_FLNCUWB::fix_config_bits32(int32_t &v, uint8_t bits) const
{
 ...
 */
bool AP_Baro_FLNCUWB::read_calibration(void)
{
    uint8_t buf[18];
...
void AP_Baro_FLNCUWB::set_config_registers(void)
{
...
bool AP_Baro_FLNCUWB::init(bool _is_dps310)
{
    if (!dev) {
        return false;
    }
    ...
    dev->set_device_type(DEVTYPE_BARO_FLNCUWB);
    ...
    dev->register_periodic_callback((1000 / 64) * AP_USEC_PER_MSEC, FUNCTOR_BIND_MEMBER(&AP_Baro_FLNCUWB::timer, void));
 ...
 */
void AP_Baro_FLNCUWB::calculate_PT(int32_t UT, int32_t UP, float &pressure, float &temperature)
{
 ...
 */
void AP_Baro_FLNCUWB::check_health(void)
{
...
void AP_Baro_FLNCUWB::timer(void)
{
    uint8_t buf[6];
    uint8_t ready;
...
void AP_Baro_FLNCUWB::update(void)
{
    ...
    _copy_to_frontend(instance, 100000 /* pressure_sum/count */, temperature_sum/count); // Pressure in pascal
    pressure_sum = 0;
    temperature_sum = 0;
    count=0;
}
